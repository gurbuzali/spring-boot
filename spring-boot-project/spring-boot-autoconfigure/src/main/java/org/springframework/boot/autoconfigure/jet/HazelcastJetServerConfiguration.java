...
/**
 * {@link EnableAutoConfiguration Auto-configuration} for Hazelcast Jet.
 * Creates a {@link JetInstance} based on explicit configuration or when a
 * default configuration file is found in the environment.
 *
 * @author Ali Gurbuz
 * @since 2.3.0
...
public class HazelcastJetServerConfiguration {
	...
	static class HazelcastJetServerConfigFileConfiguration {
		...
		JetInstance jetInstance(HazelcastJetProperties properties) throws IOException {
			...
			JetConfig jetConfig = configLocation == null ?
					ConfigProvider.locateAndGetJetConfig() : getJetConfig(configLocation);
	...
	static class JetConfigAvailableCondition extends HazelcastJetConfigResourceCondition {
		...
		JetConfigAvailableCondition() {
			super("HazelcastJet",CONFIG_ENVIRONMENT_PROPERTY,
					CONFIG_SYSTEM_PROPERTY, "file:./hazelcast-jet.xml", "classpath:/hazelcast-jet.xml",
		...
		protected ConditionOutcome getResourceOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
			if (System.getProperty(HazelcastJetClientConfiguration.CONFIG_SYSTEM_PROPERTY) != null) {
				return ConditionOutcome.noMatch(startConditionMessage().because("System property '" +
						HazelcastJetClientConfiguration.CONFIG_SYSTEM_PROPERTY + "' is set."));
			}
			if (context.getEnvironment().containsProperty(HazelcastJetClientConfiguration.CONFIG_ENVIRONMENT_PROPERTY)) {
