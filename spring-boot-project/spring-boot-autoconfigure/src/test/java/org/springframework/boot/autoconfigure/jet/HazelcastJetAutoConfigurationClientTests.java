...
class HazelcastJetAutoConfigurationClientTests {
	...
	void unknownConfigFile() {
		this.contextRunner
				.withPropertyValues("spring.hazelcast.jet.client.config=foo/bar/unknown.xml")
	...
	private static ContextConsumer<AssertableApplicationContext> assertSpecificHazelcastJetClient(String label) {
		return (context) -> assertThat(context).getBean(JetInstance.class)
											   .isInstanceOf(JetInstance.class)
	...
	private static Condition<JetInstance> labelEqualTo(String label) {
		return new Condition<>((o) -> ((JetClientInstanceImpl) o)
				.getHazelcastClient().getClientConfig().getLabels().stream()
