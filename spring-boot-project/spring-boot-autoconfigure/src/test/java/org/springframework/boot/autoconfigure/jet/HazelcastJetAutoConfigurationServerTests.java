...
class HazelcastJetAutoConfigurationServerTests {
	...
	private final ApplicationContextRunner contextRunner = new ApplicationContextRunner().withConfiguration(
			AutoConfigurations.of(HazelcastJetAutoConfiguration.class));
	...
	void explicitConfigFileWithXml() {
		this.contextRunner
				.withPropertyValues("spring.hazelcast.jet.config=org/springframework/boot/autoconfigure/jet/"
	...
	void explicitConfigUrlWithXml() {
		this.contextRunner
				.withPropertyValues("spring.hazelcast.jet.config=classpath:org/springframework/"
						+ "boot/autoconfigure/jet/hazelcast-jet-specific.xml")
				.run(assertSpecificJetServer("xml"));
	}
	...
	void explicitConfigUrlWithYaml() {
		this.contextRunner
				.withPropertyValues("spring.hazelcast.jet.config=classpath:org/springframework/"
						+ "boot/autoconfigure/jet/hazelcast-jet-specific.yaml")
				.run(assertSpecificJetServer("yaml"));
	}
	...
	void unknownConfigFile() {
		this.contextRunner
				.withPropertyValues("spring.hazelcast.jet.config=foo/bar/unknown.xml")
