...
public class HazelcastJetHealthIndicator extends AbstractHealthIndicator {
	...
	protected void doHealthCheck(Health.Builder builder) {
		submitJob();
		builder.up().withDetail("name", jetInstance.getName()).withDetail("uuid",
				jetInstance.getHazelcastInstance().getLocalEndpoint().getUuid());
	}

	/**
	 * Submits a noop {@link Job} to the Hazelcast Jet cluster and waits for it
	 * to finish without any exception. This should indicate that the Hazelcast
	 * Jet cluster is up and running.
	 */
